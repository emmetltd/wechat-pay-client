 ...
 * 微信SDK基础缓存类
 *
 * @author Emmet <115376835@qq.com>
 * @date 2019/09/03 11:55
 */
class Cache
{
    /**
    ...
    static public function set($name, $value, $expired = 0)
    {
        ...
        return self::check() && file_put_contents(self::$cachepath . $name, $data) && @chmod(self::$cachepath . $name, 0777);
    } 
    /**
     * 读取缓存
     * @param string $name
    ...
    static public function file($content, $filename = '')
    {
        ...
        if (self::check() && file_put_contents(self::$cachepath . $filename, $content) && @chmod(self::$cachepath . $filename, 0777) ) {
